!
!     Smolyak sparse grid integration library
!
!
!     used for integration of f(x1,x2,x3,...,xd) dx1 dx2 ... dxd
!
!     usage : integral = sum_i ( f( x(i,1),x(i,2),...,x(i,n) )*wi )
!
      MODULE SmolyakIntegration
      IMPLICIT NONE
!
! -----------------------------------------------------------------------------------------
!
...
!     Variables
!
      TYPE(SmolyakType) :: SSquad
!
!     Subroutines
!
      CONTAINS

      SUBROUTINE Smolyak_init(rule,dim_num,level_max)
...
!    Input, integer ( kind = 4 ) LEVEL_MAX, the level.
      ...
      INTEGER dim_num,level_max,rule,level_min,point_num
      ...
      ALLOCATE ( SSquad%wi(SSquad%n) )
      ALLOCATE ( SSquad%xi(SSquad%d,SSquad%n) )

      CALL sparse_grid ( SSquad%d, level_max, rule, SSquad%n, SSquad%wi, SSquad%xi )
...
! -----------------------------------------------------------------------------------------
!
      SUBROUTINE Smolyak_test()

!      Integrate[Integrate[Integrate[x^2 y^2 z^2, {x, -1, 1}],{y,-1,1}],{z,-1,1}] = 8/27

      INTEGER dim_num,level_max,rule,i
      REAL vsota

      rule = 5 ! Gauss-Legendre
      rule = 1 ! Gauss-Legendre
      level_max = 3
      dim_num = 3

      CALL Smolyak_init(rule,dim_num,level_max)

      vsota = 0.0D0
      DO i=1,SSquad%n
        vsota = vsota + SSquad%wi(i) * SSquad%xi(1,i)**2 * SSquad%xi(2,i)**2 * SSquad%xi(3,i)**2
      END DO 
      PRINT *,SSquad%n,SSquad%d
      PRINT *,vsota,ABS(vsota-8.0/27.0)
